...
This module is intentionally local/deterministic: it ranks candidates using
string overlap, target-profile overlap, read-across support, and existing
candidate confidence. No per-candidate LLM calls.
"""
...
# Agent infrastructure for LLM-based scoring
ROOT = Path(".")
AGENT_PATHS = {
...
def run_agent(agent_name: str, prompt: str) -> Any:
    ...
    # Import WorkflowUtils dynamically to avoid circular imports
    try:
    ...
    # Fallback: direct LLM call (simplified)
    try:
...
def similarity_scoring_node(state: AOPState) -> AOPState:
    ...
    for cand in candidates:
        ...
        try:
            ...
            if isinstance(res, dict) and "similarities" in res:
                    ...
                    # Find the score for the specific candidate we requested
    ...
    filtered = [
        c for c in updated
        if float(c.get("similarity") or 0.0) >= SIMILARITY_THRESHOLD
    ]
